...
const builder = (givenObject = {}) => {
    ...
    const performDispatch = (dispatch, name) =>
        (payload, ...args) => dispatch(thunks[name] !== undefined || reduxThunks[name] !== undefined ? getThunkActions()[name](payload, ...args) : { type: name, payload })
    ...
    const hookFunction = (...args) => {
      const pool = extractArgs(args)
      const dispatch = useDispatch();

      const reduxState = useSelector(state => {
          let data = {}

          pool.state.forEach(stateKey => {
            data[stateKey] = state[stateKey];
          })

          pool.actions.forEach(actionName => {
            data[actionName] = (...actionArgs) => {
              dispatch(thunkActions[actionName](...actionArgs))
            }
          })
          ...
          pool.combined.forEach(combinedName => {
            ...
            each(Object.keys(reduxCombinedActions[combinedName]), longName => {
                combined[longName.replace(`${combinedName}.`, '')] = (...args) => dispatch(reduxCombinedActions[combinedName][longName](...args))
            })

            each(Object.keys(givenReduxCombinedThunks[combinedName]), longName => {
                combined[longName] = (...args) => dispatch(reduxCombinedThunks[combinedName][`${combinedName}.${longName}`](...args))
            })
