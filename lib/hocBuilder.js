...
const builder = (givenObject = {}) => {
    ...
    options.pure = options.pure !== undefined ? options.pure : "pure"
    ...
    const combineHoc = (...combines) => Component => props => {
        ...
        each(combines, combineName => {
            each(Object.keys(reduxCombinedActions[combineName]), actionName => {
                const cleanName = ("" + actionName).replace(`${combineName}.`, "")
                newProps[combineName][cleanName] = (...args) => props[actionName](...args)
                delete newProps[actionName]
            })

            each(Object.keys(reduxCombinedThunks[combineName]), thunkName => {
                const cleanName = ("" + thunkName).replace(`${combineName}.`, "")
                newProps[combineName][cleanName] = (...args) => props[thunkName](...args)
                delete newProps[thunkName]
            })
        })
    ...
    each(Object.keys(givenReduxCombinedThunks), combineName => {
        ...
        each(Object.keys(combineThunks), thunk => {
            reduxCombinedThunks[combineName][`${combineName}.${thunk}`] = (...args) => (dispatch, getState) =>
                combineThunks[thunk](...args)((name, ...args) => performDispatch(dispatch, name)(...args), getState, ...args)
        })
    })
