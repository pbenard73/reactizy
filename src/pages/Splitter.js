...
const Splitter = () => (
    <section name='splitter' className='splitter'>
        ...
        <p>
            If a <code>state</code> object is defined, it will be merged into the state of the main component
        </p>

        <p>
            If a <code>hoc</code> array is provided, it will be merged inside the HOC arguments of the main Components
        </p>
        ...
        <p>
            <code>{"state = {foo: 'bar', partCount: 0}"}</code>
        </p>

        <p>And the export Hoc will correspond to : </p>
        <p>
            <code>{"export default Hoc(\"user\", \"receipeToCook\", \"login\", \"decrement\", \"logout\")(App, SubPage);"}</code>
        </p>
